...
Player::Player(string stone, Stone coord, int width, int height, string name, string history)
{
...
INPUTKEY Player::Playing(Player* opponent)
{
	while (true)
	{
		MapDraw::DrawPoint(m_strStone, m_sCursorCoord.m_iX, m_sCursorCoord.m_iY);
...
void Player::Reset(Stone coord, int width, int height)
{
	m_sCursorCoord = coord;
	m_listHistory.clear();
	m_iMapWidht = width;
